...
def _recognise_image(img, osra, frmt=None, jaggy=False, adaptive=False, unpaper=0):
...
def _image2ctab(img, osra, **kwargs):
    kwargs['frmt'] = 'sdf'
    return _recognise_image(img, osra, **kwargs)

# ----------------------------------------------------------------------------------------------------------------------


def _image2smiles(img, osra, **kwargs):
    return b'\n'.join(filter(bool, _recognise_image(img, osra, **kwargs).split(b'\n')))
